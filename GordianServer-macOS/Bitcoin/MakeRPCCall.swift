...
class MakeRpcCall {
    ...
    func command(method: String, port: String, completion: @escaping ((result: Any?, error: String?)) -> Void) {
        let nodeIp = "127.0.0.1:\(port)"
        if let password = UserDefaults.standard.string(forKey: "rpcpassword") {
            let stringUrl = "http://GordianServer:\(password)@\(nodeIp)"
            ...
            completion((nil, "No rpcpassword saved."))
        }
    }
}
